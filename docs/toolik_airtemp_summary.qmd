...
```{r}
toolik_temp_plot <- ggplot(data = toolik_weather, aes(x = as.factor(month),
...
## Turn . in data into N/A so R can read it properly using `na_if` function 

```{r}
# By renaming this column (as itself) it outputs it in the data fram and not just the console
# If we didn't do toolik_chlorophyll$active_chl_a_ug_1 <- then it would only output into console
toolik_chlorophyll$active_chl_a_ug_l <- 
na_if(toolik_chlorophyll$active_chl_a_ug_l, ".")

# Need to change this variable from a Character to Numeric in order to properly graph it
toolik_chlorophyll$active_chl_a_ug_l <- 
...
ggplot(data = toolik_chlorophyll, 
       aes(x = depth,
           y = active_chl_a_ug_l)) +
         geom_point(aes(col = active_chl_a_ug_l),
                    show.legend = FALSE) +
  scale_color_gradient(low = "blue", high = "orange") +
  theme_minimal() +
    labs(title = "Toolik Chlorophyll Concentration", 
         x = "Lake Depth (meters)",
         y = "Active Chlorophyll Concetration (micrograms per liter)")
```
